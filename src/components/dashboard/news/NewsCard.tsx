...
const NewsCard = ({ news, index }: NewsCardProps) => {
  return (
    <Card
      ...
      style={{ animationDelay: `${index * 100}ms` }}
    >
      {/* Adjusted padding to be more responsive */}
      <CardContent className="p-6 md:p-8 lg:p-10">
        <div className="flex flex-col gap-4">
          ...
          <h3 className="font-semibold text-xl md:text-2xl text-white line-clamp-2">{news.title}</h3>
          <p className="text-white/70 text-base md:text-lg line-clamp-3">{news.description}</p>
        </div>
      </CardContent>
    </Card>
  );
