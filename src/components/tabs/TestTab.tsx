...
export const TestTab = () => {
  ...
  // Find the index of the speed question
  const speedQuestionIndex = questions.findIndex(q => q.id === 'speed');
  
  // Start with the speed question (should be 0 if it's first in the array)
  const [currentQuestion, setCurrentQuestion] = useState(speedQuestionIndex >= 0 ? speedQuestionIndex : 0);

  // Ensure we filter out any already answered brake/tire questions from CompletedTestQuestions
  // when first loading the component
  useEffect(() => {
    setAnswers(prev => {
      const newAnswers = {...prev};
      // Clear brake and tire answers initially to ensure they're not shown first
      newAnswers.brakesSameForAll = "";
      newAnswers.brakesConditionAll = "";
      newAnswers.brakesConditionFront = "";
      newAnswers.brakesConditionBack = "";
      newAnswers.tireTreadSameForAll = "";
      newAnswers.tireTreadAll = "";
      newAnswers.tireTreadFront = "";
      newAnswers.tireTreadBack = "";
      return newAnswers;
    });
  }, []);
  ...
  return (
    <div className="space-y-8">
      <CompletedTestQuestions
        answers={answers}
        onEdit={() => setCurrentQuestion(speedQuestionIndex >= 0 ? speedQuestionIndex : 0)}
      />
